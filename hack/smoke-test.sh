...
#
# Every check is guarded so a failed one does not abort the run; all failures
# of an image are reported together.
...
usage() {
    local exit_code="${1:-0}"
    cat <<EOF
Usage: ${SCRIPT_NAME} [options] (--all | <image>)
...
parse_args() {
    local args
    local options="h"
    local longoptions="help,runtime:,all,keep"
...
run_smoke() {
    ...
    if ! cat <<'EOF' | "${runtime}" exec -i "${name}" tee /etc/systemd/system/molecule-test.service >/dev/null; then
[Unit]
...
EOF
        printf '  FAIL could not write the service unit\n'
        failures=$((failures + 1))
    fi
    ...
    if [[ -n ${main_pid} && ${main_pid} != 0 ]] && "${runtime}" exec "${name}" kill -0 "${main_pid}" 2>/dev/null; then
    ...
    if "${runtime}" exec "${name}" systemctl restart molecule-test.service >/dev/null 2>&1; then
        ...
        if [[ -n ${new_pid} && ${new_pid} != 0 && ${new_pid} != "${main_pid}" ]]; then
            ...
            printf '  FAIL restart did not replace the process (MainPID=%s)\n' "${new_pid}"
            failures=$((failures + 1))
        fi
    else
    ...
    if "${runtime}" exec "${name}" systemctl stop molecule-test.service >/dev/null 2>&1; then
        output=$("${runtime}" exec "${name}" systemctl is-active molecule-test.service 2>/dev/null || true)
        if [[ ${output} == inactive && -n ${new_pid} ]] &&
            ! "${runtime}" exec "${name}" kill -0 "${new_pid}" 2>/dev/null; then
    ...
    if check_ansible; then
        output=$(ansible -i "${name}," all -c "${connection}" -m ansible.builtin.setup 2>&1 || true)
        if grep -q '"ansible_system"' <<<"${output}"; then
            printf '  ok   %s\n' "$(grep -m1 -o '"ansible_distribution": "[^"]*"' <<<"${output}")"
        else
